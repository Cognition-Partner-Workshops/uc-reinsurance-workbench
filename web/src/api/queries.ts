...
export function usePortfolio() {
  ...
  const treatyIds = detailItems.flatMap((submission) => submission.treaties.map((treaty) => treaty.id));
  ...
  const catModels = useQueries({
    queries: detailItems.map((submission) => ({
      ...
      queryFn: () => api<CatModelResultModel[]>(`/api/submissions/${submission.id}/cat-model`),
    })),
  });
...
export function usePriceTreaty() {
  ...
  return useMutation({
    mutationFn: (treatyId: number) =>
      api<PricingResultModel[]>(`/api/treaties/${treatyId}/price`, { method: "POST", body: "{}" }),
...
export function useCreateSubmission() {
  ...
  return useMutation({
    mutationFn: (body: SubmissionCreateRequest) =>
      api<SubmissionModel>("/api/submissions", { method: "POST", body: JSON.stringify(body) }),
