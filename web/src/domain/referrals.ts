        ...
        .map((rule) => {
            ...
            switch (rule.kind) {
                ...
                case 1:
                    ...
                    text = hit
                        ? "Rate on line is below the minimum."
                        : "Rate on line above minimum.";
                    break;
                case 2:
                    hit = context.rateOnLine > (rule.threshold ?? 0);
                    text = hit
                        ? "Rate on line is above the maximum."
                        : "Rate on line below maximum.";
                    break;
                case 3: {
                ...
                case 4:
                    ...
                    text = hit
                        ? "Layer exceeds the underwriter's authority."
                        : "Within underwriter authority.";
                    break;
                case 5:
                    neutral = !context.serverReferralReasons;
                    hit = Boolean(
                        context.serverReferralReasons?.toUpperCase().includes("PORTFOLIO"),
                    );
                    text = neutral
                        ? "Evaluated on save."
                        : hit
                          ? "Portfolio aggregate limit breached."
                          : "Within portfolio aggregate limits.";
                    break;
                default:
