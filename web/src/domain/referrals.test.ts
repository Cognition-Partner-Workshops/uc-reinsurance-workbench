...
describe("evaluateRules", () => {
  ...
  it.each(cases)(
    "handles rule kind $kind",
    ({ kind, threshold, textValue, values, hit }) => {
  ...
  it("handles rating ordering and unknown ratings as C", () => {
    expect(evaluateRules([rule(3, undefined, "A-")], context({ cedentRating: "A-" }))[0].hit).toBe(
      false,
    );
    expect(
  ...
  it("keeps portfolio checks neutral until the server evaluates them", () => {
    ...
    const breached = evaluateRules([rule(5)], context({ serverReferralReasons: "PORTFOLIO: breach" }))[0];
