...
export default function Layout({ children, lastTreatyId }: LayoutProps) {
    ...
    const queueCount = items.filter(
        (item) =>
            item.status === SubmissionStatus.Received || item.status === SubmissionStatus.InReview,
    ).length;
    const portfolioCount = items.filter(
        (item) => item.status !== SubmissionStatus.Declined && item.treatyCount > 0,
    ).length;
        ...
        <div className="app-shell">
            <nav className="sidebar">
                ...
                <Link className={`nav-item ${active("/") ? "active" : ""}`} to="/">
                    <span className="nav-dot" /> Portfolio{" "}
                    <span className="nav-count">{portfolioCount}</span>
                </Link>
                <Link
                    className={`nav-item ${active("/submissions") ? "active" : ""}`}
                    to="/submissions"
                >
                    <span className="nav-dot" /> Submissions{" "}
                ...
                <div className="sidebar-footer">
                    ...
                    <div>
                        ...
                        <div className="muted small">
                            Authority{" "}
                            {current ? `$${(current.authorityLimit / 1e6).toFixed(1)}M` : "—"}
                        </div>
                    </div>
                </div>
            </nav>
